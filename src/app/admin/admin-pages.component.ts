  ...
  template: `
    <section class="admin admin--full" dir="rtl">
      ...
      <header class="hdr">
        ...
        <div class="actions">
          <button (click)="openHtmlEditor()" [disabled]="!currentPath()">HTML</button>
      ...
      <!-- HTML SOURCE EDITOR -->
      <div *ngIf="showHtmlEditor()" class="html-modal">
        ...
        <div class="html-box">
          ...
          <div class="actions">
            <button (click)="applyHtml()">Apply</button>
            <button (click)="showHtmlEditor.set(false)">Cancel</button>
          </div>
...
export class AdminPagesComponent {
  ...
  loadList(){
    ...
    this.http.get<PageDto[]>(this.api('/api/admin/pages')).subscribe(res=>{
      ...
      if(!this.currentPath() && res?.length){
        this.open(res[0].path);
      }
    });
  ...
  openHtmlEditor(){
    ...
    this.htmlBuffer = this.html;

    this.showHtmlEditor.set(true);

  }

  applyHtml(){

    this.html = this.htmlBuffer;

    if(this.quill){
      this.quill.clipboard.dangerouslyPasteHTML(this.html);
    }

    this.showHtmlEditor.set(false);
